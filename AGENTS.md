...
- Agent-server deferred init (warm-pool / dormant mode) is driven by `Config.deferred_init` (env `OH_DEFERRED_INIT`). The `InitService` in `openhands-agent-server/openhands/agent_server/init_router.py` owns the dormant→initializing→ready transition and is registered on `app.state.init_service` only when `deferred_init=True`; the `require_initialized` dependency, added to the `/api/*` router, returns 503 while not `ready`. Bootstrap auth for `POST /api/init` supports two mechanisms (`check_init_api_key`): **symmetric** — the caller presents `secret_key` in the `X-Init-API-Key` header (constant-time compared; the orchestrator already holds this key for encryption); and **asymmetric** — when `Config.init_public_key_file` (`OH_INIT_PUBLIC_KEY_FILE`, a PEM file of one or more public keys, typically a mounted secret) is set, the caller instead presents a short-lived ES256 JWT signed by the matching private key, as `Authorization: Bearer <jwt>`. Asymmetric lets the orchestrator hold a private key while each instance holds only the non-secret public key, so a compromised instance can't forge init. Tokens are verified by `_verify_init_jwt` with the algorithm pinned to ES256, `exp` required, and `aud` matched against the resolved audience — set as a literal via `init_token_audience` (`OH_INIT_TOKEN_AUDIENCE`), or read at startup from an env var named by `init_token_audience_env` (`OH_INIT_TOKEN_AUDIENCE_ENV`, defaulting to `AGENT_SERVER_NAME`, which the orchestrator sets per instance so an identical deploy spec gives each instance a distinct audience). An audience is **required** when asymmetric auth is enabled (boot fails if none resolves), binding a token to one instance and stopping cross-instance replay. The key file may concatenate multiple PEM blocks (each tried in turn) to support rotation. It is read once at server startup (`load_init_public_keys`, called from `api_lifespan`) and the parsed keys are cached on `app.state.init_public_keys`, so a missing/unusable key file or missing audience fails boot fast rather than rejecting every `/api/init` at runtime. Precedence fails closed: when `init_public_key_file` is set the symmetric `secret_key` is **not** accepted for init (it keeps its separate cipher role). The agent-server's 5xx exception handler rewrites `detail` on 503s, so warm-pool orchestrators should rely on the HTTP status code (not the body) when probing dormant state.
