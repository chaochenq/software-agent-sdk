...
def test_nemotron_3_ultra_550b_a55b_or_config():
    """Test that nemotron-3-ultra-550b-a55b-or (OpenRouter route) has correct config."""
    model = MODELS["nemotron-3-ultra-550b-a55b-or"]

    assert model["id"] == "nemotron-3-ultra-550b-a55b-or"
    assert model["display_name"] == "NVIDIA Nemotron-3 Ultra 550B (OpenRouter)"
    assert (
        model["llm_config"]["model"] == "litellm_proxy/nemotron-3-ultra-550b-a55b-or"
    )
    assert model["llm_config"]["temperature"] == 1.0
    assert model["llm_config"]["top_p"] == 0.95
